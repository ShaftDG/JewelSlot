function AnimationMoveInChest(targetBackward, duration)
{
    ...
    var x = (Math.random() < 0.5 ? -1 : 1) * Math.random() * 3 + targetBackward.x;
    var z = (Math.random() < 0.5 ? -1 : 1) * Math.random() * 5 + targetBackward.z;

    var cubicBezierVectors = BABYLON.Curve3.CreateQuadraticBezier(
        object.position,
        new BABYLON.Vector3((object.position.x + x)/2, object.position.y+10, object.position.y-5),
